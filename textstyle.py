...
# from object_detection import get_object_detections
...
# def get_object_detections(image_path):
#     """Detect objects using YOLOv8"""
#     results = yolo_model(image_path)
#     object_mask = np.zeros_like(cv2.imread(image_path, cv2.IMREAD_GRAYSCALE))

#     for result in results:
#         for box in result.boxes.xyxy:
#             x1, y1, x2, y2 = map(int, box[:4])
#             cv2.rectangle(object_mask, (x1, y1), (x2, y2), 255, -1)  # Fill detected objects

#     pil_mask = Image.fromarray(object_mask)
...
# def render_text_with_pango(text, font, font_size, width, height):
#     """
#     Renders text using Pango and Cairo with proper text wrapping and anti-aliasing.
#     """
...
#     layout.set_width(width * pango.SCALE)  # Pango uses scaled units
#     layout.set_wrap(pango.WRAP_WORD)
#     layout.set_text(text)

#     # Move to position and render text
#     ctx.move_to(10, 10)  # Adjust position
...
def generate_styled_image(text, image, festival):
    ...
    if (festival and festival.lower() in ["holi", "dashain"]) or (coordinates == (None, None, None, None)):
        ...
        # Optionally, you can add some text or labels in the blank space here
        # cv2.putText(new_img, text, (x+padding, y+padding), cv2.FONT_HERSHEY_SIMPLEX, 1, dominant_color, 2)
        text_image = new_img
        # return new_img
        ...
        font = ImageFont.truetype(font_style, font_size)
        alpha_value = 150
        # image_with_box = add_transparent_box(image_copy.convert("RGBA"), coordinates, dominant_color, alpha_value)
        # text_image  = image_with_box
        text_image = image_copy
